...
class NotificationServiceImplementationTest {
    ...
    void testCreateNotification_Success() {
        ...
        assertEquals(user.getName(), response.getRecipientName());
    }
    ...
    void testSendNotification_Success() {
        ...
        when(notificationRepo.save(captor.capture())).thenAnswer(invocation -> {
            Notification n = invocation.getArgument(0);
            n.setId(100L);
            return n;
        });

        when(notificationRepo.findById(100L)).thenAnswer(invocation -> Optional.of(captor.getValue()));
        ...
        assertEquals("Reminder", saved.getMessage());
    ...
    void testScheduleNotification_Success() {
        ...
        verify(rabbitTemplate, times(1))
                .convertAndSend(anyString(), anyString(), eq(request), any(MessagePostProcessor.class));
    }
}
